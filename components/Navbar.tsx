...
import { resumeDriveLink, userEmail } from '@/shared/UserData';
  ...
  return (
    <div className="w-full shadow-navbarShadow h-20 lg:h-[12vh] sticky top-0 z-50 bg-bodyColor px-4">
      <div className="max-w-container h-full mx-auto py-1 font-titleFont flex items-center justify-between">
        ...
        >
          <GiCondorEmblem
            className="w-10 h-10 ml-[30%]
                        hover:text-textGreen cursor-pointer hover:transition-all duration-300"
          />
        </motion.div>
        <div className="hidden mdl:inline-flex items-center gap-7">
          <ul className="flex text-[13px] gap-7">
            <NavBarItemComponent
              initialPosition={{ y: -10, opacity: 0 }}
              finalPosition={{ y: 0, opacity: 1 }}
            />
          </ul>
          <ResumeComponent />
        </div>
        ...
        {showMenu && (
          <div
            ref={(node: HTMLDivElement) => (ref.current = node)}
            ...
            >
              ...
              <div className="flex flex-col items-center gap-7">
                <ul className="flex flex-col text-base gap-7">
                  <NavBarItemComponent
                    initialPosition={{ x: 10, opacity: 0 }}
                    finalPosition={{ x: 0, opacity: 1 }}
                  />
                </ul>
                <ResumeComponent />
                <Footer />
